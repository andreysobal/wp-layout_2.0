	...
	<section class="section main-block">
		<div class="wrapper flex-wrapper flex-wrapper_nowrap flex-wrapper_align-start">
			<div class="section__content content">
				<div class="text text_capit text_black text_super-hight text_separated">Результат поиска по запросу <br> 
					<span class="quotes">"</span> <?php the_search_query() ?> <span class="quotes">"</span>:
				</div>
				<?php if (have_posts()) {
					while (have_posts()) { 
						the_post();
				...
				<?php } else { ?>
					<div class="section__title text text_capit text_black text_super-hight">По Вашему запросу ничего не найдено.</div>
				<?php } //ends if ?>
			</div>
			<?php get_sidebar(); ?>
		</div>
